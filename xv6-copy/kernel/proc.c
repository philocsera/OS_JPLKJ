...
found:
  ...
  // LLM-for-OS observability fields.
  p->trace_mask = 0;
  p->run_ticks = 0;
  p->sleep_ticks = 0;
...
kfork(void)
{
  ...
  *(np->trapframe) = *(p->trapframe);
  ...
  // Inherit priority from parent to child
  np->priority = p->priority;

  // Inherit trace mask, but reset per-process tick counters for the child.
...
scheduler(void)
{
  ...
  for(;;){
    ...
    // Side effect: any SLEEPING proc seen during the scan gets its
    // sleep_ticks bumped, so we accumulate observation samples without
    // a separate per-tick walker.
    struct proc *best = 0;
...
// Fill a caller-provided array with a snapshot of the proc table.
